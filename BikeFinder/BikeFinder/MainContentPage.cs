...
namespace BikeFinder
{
	public class MainContentPage : ContentPage
	{
		...
		public MainContentPage ()
		{
			...
			//Label cityListLabel = new Label()
			//{
			//	Text = "Choose a city or network",
			//	FontSize = 16,
			...
			map = new Xamarin.Forms.Maps.Map(initialSpan);
		...
		void CityChosen (Network chosen)
		{
			cityListStackLayout.IsVisible = false;
		...
		public void BackFromPin (Network network)
		{
			CityChosen (network);
		...
		void route ()
		{
			altSubMenuGrid.IsVisible = false;
			pinDetailView.IsVisible = false;
			Launcher.OpenAsync(new Uri (string.Format (Constants.AppleMapsURL,
		...
		async void showMap (Network chosen)
		{
			...
			try {
				...
				chosen.url = chosen.url.Replace("http://", "https://");
		...
		async void refresh ()
		{
			CityChosen(Networks.Instance.CurrentNetwork);

			subMenuGrid.IsVisible = true;			
		}
		...
		void PopulateTable ()
		{
			if (Networks.Instance.NetworkList != null && Networks.Instance.NetworkList.Count > 0) {
				...
				cityTable.HasUnevenRows = false;
				cityTable.BackgroundColor = Color.White;
				cityTable.RowHeight = 48;
			} else {
		...
		void OpenWebsite ()
		{
			...
			subMenuGrid.IsVisible = false;
			aboutView.IsVisible = false;
			Launcher.OpenAsync(new Uri (Constants.CityBikesURL));
		}
